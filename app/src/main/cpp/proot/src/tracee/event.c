...
void print_backtrace(pid_t pid) {
    ...
    if (!addr_space) {
        note(NULL, ERROR, SYSTEM, "unwind: failed to create addr space");
        return;
    }
    ...
    if (rctx == NULL) {
        note(NULL, ERROR, SYSTEM, "unwind: _UPT_create failed");
        return;
    }

    if (unw_init_remote(&cursor, addr_space, rctx)) {
        note(NULL, ERROR, SYSTEM, "unwind: unw_init_remote failed");
        return;
    }
    ...
    do {
        ...
        if (unw_get_proc_name(&cursor, bt_func_buf, 256, &off) == 0) {
            note(NULL, ERROR, SYSTEM, "backtrace #%d 0x%lx %s", idx++, ip, bt_func_buf);
        }
        else {
            note(NULL, ERROR, SYSTEM, "backtrace #%d 0x%lx", idx++, ip);
        }
...
void dump_mem_info(pid_t pid) {
    char path[256];
    snprintf(path, sizeof(path), "/proc/%d/maps", pid);

    FILE *fp = fopen(path, "r");
    if (fp == NULL) {
        note(NULL, ERROR, SYSTEM, "Error opening /proc/[pid]/map");
        return;
    }

    char line[1024];
    note(NULL, ERROR, SYSTEM, "%s begin", path);
    while (fgets(line, sizeof(line), fp) != NULL) {
        note(NULL, ERROR, SYSTEM, "%s", line);
    }
    note(NULL, ERROR, SYSTEM, "%s end", path);
}
	...
	while (1) {
		...
		if (pid < 0) {
			...
			break;
                ...
                // get backtrace of process, SIGSEGV, SIGBUS，SIGABRT
                if (siginfo.si_signo == 11 || siginfo.si_signo == 7 || siginfo.si_signo == 6) {
                    print_backtrace(pid);
                    dump_mem_info(pid);
                }
